...
| 2 | Offer re-surfacing on duplicate delivery (Swift re-surfaces latest-wins; pin Rust returns nil on idempotent re-delivery) | silent — staple idempotency is pinned (wire-format.md), offer re-surface on duplicate delivery is not | n/a (acceptor-side, host-facing) | open — pending correlation of mismatch sites with duplicate deliveries | open | likely yes — hosts key offers by digest; a re-surfaced duplicate folds to the same digest (idempotent) | **Open** — next investigation |
