...
| 2 | Offer surfacing asymmetry (Swift surfaces `queuedProposal` on every successful decrypt; pin Rust surfaces `proposal: Some` on the main path, `nil` on other outcomes) | defined in mechanism — duplicate delivery is expected protocol behavior, and duplicate rejection rides on **application-message consumption** (ratchet generation), not ciphertext-level duplicate detection | n/a | **identical, evidenced** — per-frame `(offerSurfaced, errClass)` matches on every frame delivered in both directions: 9 duplicate-app re-deliveries rejected `.stale` with no offer on either engine (the app-consumption model, validated directly); 408/325 fresh decrypts surfaced offers on both; 0 fresh-decrypt failures on either engine; 0 non-app mismatches | n/a — no divergence exists | **yes, via app consumption** — duplicates are cheap symmetric rejects | **Closed** — no independent divergence; noise resolved via a handling-only comparison. The mixed-pair comparison now excludes cross-direction offer/frame-PRESENCE divergence (offer-presence divergences are counted in a per-run summary line, not failed): `queueProposal`/`deliver` call-count differences and `offeredDigest` (hadOffer) presence mismatches are classified `offer-presence-divergence`. Per-frame HANDLING mismatches still fail. Presence comparison stays live in the pure-pair oracles (within-engine, where it is meaningful). 12-seed pin: ~528 presence divergences excluded across seeds (17–36/seed); the red set collapsed to row 3. (b) engine-state-independent rotation scheduling filed as a follow-up |
| 3 | Post-restore frame handling (`remoteCommitApplied` 152, `errorClass` 92, `appPayloads` 76 — Rust `DecryptionFailed`/`Mls`/`unsupportedFrameTag` after restore — plus 23 side-band leg-presence cascades attributed here by the cascade rule) | underspecified — restore fail-closed and the `dependsOnSeq` durability contract are pinned; what a restored session does with frames delivered before the restore point is implied, not stated | open | open | open | open — fail-closed restore → re-establish is the designed recovery; question is whether anything worse than re-establish occurs | **Open** — the sole remaining red set after rows 1–2 closed: 189 finding lines across the 12-seed pin run |
...
- **Row 2 comparison contract (option a):** mixed-pair comparison is HANDLING-ONLY for
  offers — cross-direction offer/frame PRESENCE divergence is excluded and counted, because a
  fold/deliver call-count difference is by construction a presence difference (the op folds
  whatever is present). `offeredDigest` is compared as presence, so its mismatch is excluded
  too. Per-frame handling (both sides had the frame), payloads, `remoteCommitApplied`,
  `errorClass` and the probe invariants still fail. Presence comparison remains in the
  pure-pair oracles.
