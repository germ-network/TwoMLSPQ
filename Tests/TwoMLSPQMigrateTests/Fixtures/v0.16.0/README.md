# v0.16.0 session-row fixtures

Golden `PQSession.Persisted` rows written by TwoMLSPQ v0.16.0 (binding contract 33), pinning
that a pre-`swift_export` archive still restores and keeps messaging under the current
engine. `LegacyRowFixtureTests.swift`, in this same test target, restores these through the
raw binding and drives a liveness exchange + a committing round each way.
...
tag's own source, not the released asset. `rust/Cargo.lock` is gitignored and was not tracked at
the tag, so crates.io dependencies were resolved fresh at build time — but the archive encoding
is still pinned because mls-rs (including `mls-rs-codec`) is fixed by git rev `b43703f` in
`rust/Cargo.toml`'s `[patch.crates-io]`, and the generated binding is byte-identical to the tag's
vendored `Sources/TwoMLSPQBinding/two_mls_pq.swift`.

`Generator.swift.txt` is the generator test source, kept here as a non-compiled reference (it
targets v0.16.0's API and does not build against this repo). Regenerating reproduces the FLOW
these fixtures encode, not their exact bytes — every identity is randomly minted, so a fresh run
produces different (but equivalent) rows. To regenerate:

```
git worktree add <a new directory> v0.16.0    # any path outside this repo
cd <that directory>
