...
**Sequencing:** a wire-format reassessment (always-staple the send-group commit;
one message-frame shape; retagging) is planned separately and **lands first**;
header encryption is then applied on top of the new frames. The header layer's
rules are per-*stream* (message path vs. PQ side-band), not per-tag, so this
design carries over intact — but the tag values named below refer to the current
[Wire Format](./wire-format.md) and will be finalized against the reworked one
...
|---|---|---|
| frame tag (`0x01`–`0x17`) | first byte of every tagged frame | frame kind: establishment vs. rotation vs. PQ side-band activity (bootstrap, ratchet, re-key) |
...
- TwoMLSPQ replaced stapling with explicit length-prefixed tagged frames. The
  ...
  the PARTIAL handler applies the commit and surfaces the stapled proposal's digest
...
- **Message-path frames** (0x01/0x03/0x05/0x09/bare-MLS — `encrypt`'s output: A.2
  ratchet, rotation bundles, the acceptor's stapled-welcome frame): seal under
  `HeaderKey(recv_group, current classical epoch)`.
- **PQ side-band frames** (0x0B–0x17): seal under `HeaderKeyPQ(recv_group,
  ...
  both the responder frames surfaced by `pq_take_pending_outbound` (0x0D, 0x13,
  0x17) **and the initiator frames returned directly by `pq_ratchet_begin`
  (0x0B), `pq_bootstrap_begin` (0x11), and `pq_rekey_begin` (0x15)** — the latter
  ...
  - *Pre-A.4 fallback:* before the bootstrap completes, only one shared PQ group
    exists (Group_A.pq) — the initiator's BOOTSTRAP_KP (0x11, and any resend)
...
- The acceptor always has a recv group from `accept()` onward, so *every* acceptor
  frame — including the first, `STAPLED_WELCOME`-tagged one — is symmetric, sealed
  under `HeaderKey(Group_A, join epoch)`. The initiator can open it: see below.
- **Seal timing:** symmetric frames are sealed **on exit** — at the boundary where
  bytes leave the library — so the acceptor's `pending_outbound` welcome is stapled
  raw into the STAPLED_WELCOME frame and the whole frame sealed once (no double
  sealing). The initiator's first frame is the one sealed-at-rest value (enveloped
  when composed, stored sealed, `pending_outbound` returns it as-is).
...
4. **Bob's first frame** — `STAPLED_WELCOME(APQWelcome_B, inner)` sealed under
...
- **`open_incoming(blob) -> OpenedFrame { kind, frame }`** — new session method: one
  ...
  its kind. `kind` must classify the **full** tag set — establishment (0x01/0x09),
  message-path (0x03/0x05/bare-MLS), and PQ side-band (0x0B–0x17) — so the host can
  route to `process_incoming` / `pq_ratchet_*` / `pq_rekey_*` / `pq_bootstrap_*` as
  today; those entry points keep their plaintext-frame signatures. The key family
  that opened the blob pre-sorts side-band from message-path; the inner tag byte
  refines it. (The PQ-tag classifier exported on the `llm/export-pq-frame-kind`
  branch is a precursor covering only 0x0B–0x17; this needs the superset, applied
  *after* opening.) `forwarded(spawn_token)` is untouched — it already takes the
  token, not bytes.
