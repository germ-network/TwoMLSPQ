...
- **PQ side-band frames** (0x05–0x11 and 0x17): seal under `HeaderKeyPQ(recv_group,
  ...
  `pq_take_pending_outbound` (the responder's 0x07, 0x0D, 0x11, and both binds — 0x09 and
  0x17 — which are initiator frames despite being surfaced rather than returned) **and
...
`PqSideBand { PqFrameKind }` for 0x05–0x11 and 0x17) and returns `OpenedFrame { kind, frame }`;
...
- **`open_incoming(blob) -> Option<OpenedFrame { kind, frame }>`** — the session
  ...
  `PqSideBand { PqFrameKind }` for 0x05–0x11 and 0x17), or `None` if no window key opens it.
