...
`InvitationSpent`, `ArchiveInvalid`, `UnsupportedCipherSuite`, `CipherSuiteMismatch`). mls-rs
error types never cross the FFI boundary. `CipherSuiteMismatch` is raised when a peer key
