...
| `ESTABLISHMENT_HANDOFF_TAG` | `0x0B` | Born-dedicated establishment staple: `[0x0B][u32-LE len][signed handoff blob][u32-LE len][APQWelcome_B]` — wraps the *unmodified* welcome next to the acceptor's opaque host-signed delegation. A message-path staple form (rides the `0x03` staple slot and standalone delivery), but banded here because it exists only during establishment |
